...
1. The home screen visibly matches the Nahida dream-garden mood: illustrated hero, leaf emblem, luminous butterfly, four botanical element tiles, portrait decoration, native Codex suggestion cards, and skinned native composer.
2. The sidebar uses its dedicated dark-green character crop rather than merely changing the accent color.
...
- Narrower window: accept Codex's native responsive reduction to two or three suggestion cards; no essential control is covered and portrait/element decorations may intentionally hide.
- Normal task: messages remain readable and composer does not overlap content.
- Inspect the sidebar, header, hero edges, extracted card icons, card labels, composer controls, scrollbar, empty scene, and portrait decoration.
...
- Serve a fake `app://` CDP target or remote/mismatched WebSocket URL and confirm both launcher and injector reject it. Rebuild the verified Codex browser on the same port with a new Browser ID and confirm the old watcher delegates to `start-dream-skin.ps1`, exits without direct reconnection, and a newly verified watcher restores the theme. Reuse the port from an unverified process and confirm recovery fails closed.
...
- Close Codex without restore and confirm the Browser identity anchor closes, the watcher exits after the bounded wait, and Codex is not automatically relaunched.

## Automated checks

- `tests/run-tests.ps1`: strict UTF-8/no-BOM writes, UTF-16 rejection, LF/CRLF preservation, concurrent-write detection, exact backup/recovery, `[desktop]`-scoped restore, ambiguous TOML rejection, non-ASCII paths, Appx/state identity, argument quoting, payload construction, theme-asset contracts, Browser ID, loopback URL rejection, and renderer isolation for transparent auxiliary windows.
