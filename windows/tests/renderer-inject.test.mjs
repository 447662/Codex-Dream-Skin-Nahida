...
function createFixture({
  ...
  const home = {
    ...
    querySelector(selector) {
      ...
      if (selector === ".composer-surface-chrome, [data-codex-composer]") {
        return homeComposerPresent ? homeComposerChrome : null;
      }
...
assert.equal(scheduledPage.shellMainClasses.has("dream-route-shell"), false);
...
assert.equal(pluginDetailPage.shellMainClasses.has("dream-route-shell"), false);
...
assert.equal(archivedChatsPage.shellMainClasses.has("dream-route-shell"), false);
