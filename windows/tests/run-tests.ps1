...
try {
  ...
  if (-not (Wait-DreamSkinProcessExit -ProcessId 2147483647 -TimeoutSeconds 1)) {
    throw 'Process-exit wait did not accept an already absent process.'
  }
  $waitParameterRejected = $false
  try { $null = Wait-DreamSkinProcessExit -ProcessId 0 -TimeoutSeconds 1 } catch { $waitParameterRejected = $true }
  if (-not $waitParameterRejected) { throw 'Process-exit wait accepted an invalid PID.' }

  $managerStateRoot = Join-Path $temporaryRoot 'manager-preference'
  $managerEngine = Join-Path $managerStateRoot 'engine'
  $managerScripts = Join-Path $managerEngine 'scripts'
  $managerActiveTheme = Join-Path $managerStateRoot 'active-theme'
  New-Item -ItemType Directory -Path $managerScripts,$managerActiveTheme -Force | Out-Null
  Write-DreamSkinUtf8FileAtomically -Path (Join-Path $managerEngine 'VERSION') -Content "1.5.9`r`n"
  Write-DreamSkinUtf8FileAtomically -Path (Join-Path $managerScripts 'start-dream-skin.ps1') `
    -Content "param()`r`n"
  Write-DreamSkinUtf8FileAtomically -Path (Join-Path $managerActiveTheme 'theme.json') `
    -Content "{}`r`n"
  $preferredManager = Get-DreamSkinPreferredManagerStartScript -StateRoot $managerStateRoot
  if (-not (Test-DreamSkinPathEqual -Left $preferredManager `
    -Right (Join-Path $managerScripts 'start-dream-skin.ps1'))) {
    throw 'An installed, enabled CodexDreamSkin manager was not selected for launch handoff.'
  }
  Write-DreamSkinUtf8FileAtomically -Path (Join-Path $managerStateRoot 'paused') `
    -Content "paused`r`n"
  if ($null -ne (Get-DreamSkinPreferredManagerStartScript -StateRoot $managerStateRoot)) {
    throw 'A paused CodexDreamSkin manager still intercepted the Nahida launch path.'
  }
  ...
  $installScript = Get-Content -LiteralPath (Join-Path $Root 'scripts\install-dream-skin.ps1') -Raw -Encoding utf8
  $restoreScript = Get-Content -LiteralPath (Join-Path $Root 'scripts\restore-dream-skin.ps1') -Raw -Encoding utf8
  $launcherSource = Get-Content -LiteralPath (Join-Path $Root 'scripts\launch-dream-skin.mjs') -Raw -Encoding utf8
  $autoStartLauncherSource = Get-Content -LiteralPath (Join-Path $Root 'scripts\launch-dream-skin-at-login.mjs') -Raw -Encoding utf8
  $desktopShortcutToken = "Join-Path `$desktop 'Codex.lnk'"
  if (-not $installScript.Contains($desktopShortcutToken) -or
    $installScript -notmatch 'desktop-codex\.before-dream-skin\.lnk' -or
    $installScript -notmatch 'launch-dream-skin\.mjs' -or
    $installScript -notmatch 'launch-dream-skin-at-login\.mjs' -or
    $installScript -notmatch "Startup'" -or
    $installScript -notmatch 'Codex Dream Skin Auto Start\.lnk' -or
    $installScript -notmatch 'Get-AuthenticodeSignature' -or
    $installScript -notmatch "@\('Codex\.lnk', 'Codex Dream Skin\.lnk'\)" -or
    $installScript -match '\$desktopShortcut\.TargetPath = \$powershell') {
    throw 'Installer no longer preserves and replaces the canonical desktop Codex shortcut safely.'
  }
  if ($launcherSource -notmatch 'start-dream-skin\.ps1' -or
    $launcherSource -notmatch 'process\.env\.LOCALAPPDATA' -or
    $launcherSource -notmatch '"-ProfilePath"' -or
    $launcherSource -notmatch '"-PromptRestart"' -or
    $launcherSource -notmatch 'args\.length !== 2' -or
    $launcherSource -notmatch 'spawnSync\(powershell' -or
    $launcherSource -notmatch 'launcher-start\.out\.log' -or
    $launcherSource -notmatch 'launcher-start\.err\.log' -or
    $launcherSource -notmatch 'stdio: \["ignore", stdoutHandle, stderrHandle\]' -or
    $launcherSource -notmatch 'fs\.closeSync\(stdoutHandle\)' -or
    $launcherSource -notmatch 'readLogTail\(startStdoutPath\)' -or
    $launcherSource -notmatch 'launcher\.log' -or
    $launcherSource -notmatch 'fs\.renameSync') {
    throw 'Signed Node launcher no longer provides the bounded Dream Skin start path.'
  }
  if ($autoStartLauncherSource -notmatch 'START_DELAY_MS = 5_000' -or
    $autoStartLauncherSource -notmatch 'RETRY_DELAYS_MS = \[0, 5_000, 10_000, 20_000\]' -or
    $autoStartLauncherSource -notmatch 'OPERATION_LOCK_MESSAGE' -or
    $autoStartLauncherSource -notmatch '"-RestartExisting"' -or
    $autoStartLauncherSource -notmatch '"-AllowDeferredVerify"' -or
    $autoStartLauncherSource -notmatch 'process\.env\.LOCALAPPDATA' -or
    $autoStartLauncherSource -notmatch 'spawnSync\(launch\.powershell' -or
    $autoStartLauncherSource -notmatch 'stdio: \["ignore", stdoutHandle, stderrHandle\]' -or
    $autoStartLauncherSource -notmatch 'autostart\.log' -or
    $autoStartLauncherSource -notmatch 'autostart-start\.out\.log' -or
    $autoStartLauncherSource -notmatch 'autostart-start\.err\.log' -or
    $autoStartLauncherSource -notmatch 'options\.selfTest' -or
    $autoStartLauncherSource -match 'MessageBox|showError') {
    throw 'Login launcher no longer provides delayed, silent, bounded lock recovery.'
  }
  $startScript = Get-Content -LiteralPath (Join-Path $Root 'scripts\start-dream-skin.ps1') -Raw -Encoding utf8
  $injectorSource = Get-Content -LiteralPath (Join-Path $Root 'scripts\injector.mjs') -Raw -Encoding utf8
  $defaultProfileToken = "Join-Path `$StateRoot 'Profile-v3'"
  if (-not $startScript.Contains($defaultProfileToken) -or
    $startScript -notmatch '--user-data-dir' -or
    $startScript -notmatch '\[switch\]\$AllowDeferredVerify' -or
    $startScript -notmatch 'Dream Skin verification is deferred' -or
    $startScript -notmatch 'verify-start\.out\.log' -or
    $startScript -notmatch 'verify-start\.err\.log' -or
    $startScript -notmatch 'Start-Process -FilePath \$node\.Path -ArgumentList \$verifyArgs' -or
    $startScript -notmatch "'--timeout-ms', '120000'") {
    throw 'Desktop launch no longer defaults to the dedicated CDP user-data directory.'
  }
  if ($startScript -notmatch 'Get-DreamSkinPreferredManagerStartScript' -or
    $startScript -notmatch 'OperationLockTimeoutMilliseconds' -or
    $startScript.IndexOf('Get-DreamSkinPreferredManagerStartScript') -gt
      $startScript.IndexOf('Enter-DreamSkinOperationLock -WaitMilliseconds')) {
    throw 'Nahida launch no longer yields to an enabled CodexDreamSkin manager before taking the operation lock.'
  }
  $managerTrayPath = Join-Path (Split-Path -Parent $Root) `
    'CodexDreamSkin\payload\scripts\tray-dream-skin.ps1'
  if (Test-Path -LiteralPath $managerTrayPath -PathType Leaf) {
    $managerTraySource = Get-Content -LiteralPath $managerTrayPath -Raw -Encoding utf8
    if ([regex]::Matches(
      $managerTraySource,
      'Start-DreamSkinPowerShell -Script \$startScript'
    ).Count -lt 5) {
      throw 'CodexDreamSkin tray switches no longer start the manager runtime after changing themes.'
    }
  }
  if ($startScript -notmatch '\[int\]\$RecoveryDelayMs = 0' -or
    $startScript -notmatch 'Start-Sleep -Milliseconds \$RecoveryDelayMs' -or
    $startScript -notmatch '\[int\]\$OperationLockWaitMs = 0' -or
    $startScript -notmatch 'Enter-DreamSkinOperationLock -WaitMilliseconds \$OperationLockWaitMs' -or
    $injectorSource -notmatch 'waitForReplacementBrowser' -or
    $injectorSource -notmatch 'listAppTargets\(options\.port, options\.browserId\)' -or
    $injectorSource -notmatch 'path\.resolve\(here, "start-dream-skin\.ps1"\)' -or
    $injectorSource -notmatch 'path\.win32\.join\(stateRoot, "Profile-v3"\)' -or
    $injectorSource -notmatch 'RECOVERY_OPERATION_LOCK_WAIT_MS = 180000' -or
    $injectorSource -notmatch '"-OperationLockWaitMs"' -or
    $injectorSource -notmatch '"-RestartExisting"' -or
    $injectorSource -notmatch '"--close-browser"' -or
    $injectorSource -notmatch 'method: "Browser\.close"' -or
    $startScript -notmatch 'Stop-DreamSkinLaunchedSession' -or
    $startScript -notmatch 'Get-DreamSkinVerifiedCdpIdentity -Port \$SessionPort -Codex \$Codex' -or
    $injectorSource -notmatch 'browser-recovery\.log' -or
    $injectorSource -match 'connectBrowserIdentityAnchor\(options\.port, replacementBrowserId\)') {
    throw 'Injector no longer delegates browser rebuild recovery through the bounded verified start path.'
  }
  if ($restoreScript -notmatch 'desktop-codex\.before-dream-skin\.lnk' -or
    $restoreScript -notmatch 'Copy-Item[^\r\n]+\$desktopCodexBackup[^\r\n]+\$desktopCodex' -or
    $restoreScript -notmatch 'Codex Dream Skin Auto Start\.lnk') {
    throw 'Restore no longer puts the original desktop Codex shortcut back.'
  }

  $node = Get-DreamSkinNodeRuntime
  $autoStartSelfTestOutput = @(& $node.Path (Join-Path $Root 'scripts\launch-dream-skin-at-login.mjs') --self-test 2>&1)
  if ($LASTEXITCODE -ne 0) { throw 'Login launcher self-test failed.' }
  $autoStartSelfTest = ($autoStartSelfTestOutput -join "`n") | ConvertFrom-Json
  if (-not $autoStartSelfTest.pass -or -not $autoStartSelfTest.restartExisting -or
    -not $autoStartSelfTest.deferredVerify -or
    -not $autoStartSelfTest.logsUseFileHandles -or $autoStartSelfTest.initialDelayMs -ne 5000 -or
    @($autoStartSelfTest.retryDelaysMs).Count -ne 4) {
    throw 'Login launcher self-test returned an unsafe configuration.'
  }
