...
- Source ≤ 50 MB; each prepared file ≤ 16 MB; one multi-image theme ≤ 32 MB total
...
- Bundled themes may also set `images.sidebar`, `images.portrait`, `images.decorations`, and `images.scene` in `theme.json`
- Custom themes created from one image remain compatible: missing image slots automatically reuse the main `image`
