...
if (mode === "reset-demo") {
  ...
  console.log("Restored the bundled default preset.");
  process.exit(0);
}
...
async function preparedImage(option, fallback) {
  const candidate = valueFor(option, fallback);
  const filename = path.basename(candidate);
  if (filename !== candidate || !/\.(?:png|jpe?g|webp)$/i.test(filename)) {
    throw new Error(`${option} must be a PNG, JPEG, or WebP filename in the theme directory.`);
  }
  const imagePath = path.join(outputDir, filename);
  const imageStat = await fs.stat(imagePath);
  if (!imageStat.isFile() || imageStat.size < 1 || imageStat.size > 16 * 1024 * 1024) {
    throw new Error(`The prepared ${option} must be non-empty and no larger than 16 MB.`);
  }
  return filename;
}

const image = await preparedImage("image", "background.jpg");
const images = {
  sidebar: await preparedImage("sidebar-image", image),
  portrait: await preparedImage("portrait-image", image),
  decorations: await preparedImage("decorations-image", image),
  scene: await preparedImage("scene-image", image),
};
