...
- 内置“森息工坊”主题：去水印森林主背景、侧栏氛围图、人物装饰、透明元素图集和空任务场景
- `theme.json` 支持 `sidebar`、`portrait`、`decorations`、`scene` 可选图片；旧单图主题自动回退到主图
- 首页四张建议卡使用透明元素图集切片，品牌徽记与晶蝶装饰复用同一图集，避免持续增加独立资源槽位

### 改进

- 深色与浅色壳统一采用森林绿、青绿、嫩绿与暖橙多维配色，并补齐弹层、代码块、滚动条和空状态样式
- payload 校验新增多资源数量/体积报告；自动化测试锁定图片尺寸、格式、透明通道和目录边界
