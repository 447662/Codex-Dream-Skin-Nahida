((cssText, artDataUrls, themeConfig) => {
  ...
  const THEME_VARIABLES = [
    ...
    "--dream-skin-art", "--dream-skin-sidebar-art", "--dream-skin-portrait-art",
    "--dream-skin-decor-art", "--dream-skin-scene-art",
  ...
  for (const value of new Set(Object.values(previous?.artUrls || {}))) URL.revokeObjectURL(value);

  const artInputs = typeof artDataUrls === "string" ? { hero: artDataUrls } : (artDataUrls || {});
  const heroInput = artInputs.hero || "data:image/png;base64,";
  const artUrls = {};
  const urlByInput = new Map();
  for (const slot of ["hero", "sidebar", "portrait", "decorations", "scene"]) {
    const input = artInputs[slot] || heroInput;
    if (!urlByInput.has(input)) {
      const comma = input.indexOf(",");
      const mime = /^data:([^;,]+)/.exec(input)?.[1] || "image/png";
      const binary = atob(input.slice(comma + 1));
      const bytes = new Uint8Array(binary.length);
      for (let index = 0; index < binary.length; index += 1) bytes[index] = binary.charCodeAt(index);
      urlByInput.set(input, URL.createObjectURL(new Blob([bytes], { type: mime })));
    }
    artUrls[slot] = urlByInput.get(input);
  }
  ...
  const applyTheme = (root, shell) => {
    ...
    const accent = colors.accent || "#86d75f";
    const accentAlt = colors.accentAlt || accent;
    const secondary = colors.secondary || "#58c7c3";
    const highlight = colors.highlight || "#f0a24a";
    ...
    if (shell === "light") {
      ...
      variables = {
        ...
        "--ds-text": "#17251d",
        "--ds-muted": "#566b5e",
        "--ds-line": colors.line || "rgba(63, 126, 82, .22)",
      };
    } else {
      variables = {
  ...
  const ensure = () => {
    ...
    root.style.setProperty("--dream-skin-art", `url("${artUrls.hero}")`);
    root.style.setProperty("--dream-skin-sidebar-art", `url("${artUrls.sidebar}")`);
    root.style.setProperty("--dream-skin-portrait-art", `url("${artUrls.portrait}")`);
    root.style.setProperty("--dream-skin-decor-art", `url("${artUrls.decorations}")`);
    root.style.setProperty("--dream-skin-scene-art", `url("${artUrls.scene}")`);
    ...
    const primaryMain = home || document.querySelector('[role="main"]');
    const emptyState = Boolean(primaryMain && !home &&
      !primaryMain.querySelector('article, [data-message-author-role]'));
    for (const candidate of document.querySelectorAll(".dream-skin-empty-state")) {
      if (candidate !== primaryMain) candidate.classList.remove("dream-skin-empty-state");
    }
    primaryMain?.classList.toggle("dream-skin-empty-state", emptyState);
    ...
    if (!chrome || chrome.parentElement !== document.body) {
      ...
      chrome.innerHTML = `
        ...
        <div class="dream-skin-portrait"></div>
        <div class="dream-skin-empty-motif"></div>
        <div class="dream-skin-butterfly"></div>
  ...
  const cleanup = () => {
    ...
    document.querySelectorAll(".dream-skin-empty-state").forEach((node) => node.classList.remove("dream-skin-empty-state"));
    ...
    for (const value of new Set(Object.values(state?.artUrls || {}))) URL.revokeObjectURL(value);
