...
"$NODE" "$ROOT/tests/theme-assets.test.mjs"
...
EXPECTED_VERSION='1.2.0 "nightly"'
...
"$NODE" -e '
  ...
  if (!value.pass || value.themeName !== "测试主题" || value.imageBytes < 1 || value.assetCount !== 1) process.exit(1);
' "$PAYLOAD_JSON"
